...
This feature creates a production-style demonstration environment showcasing SPIRE/SPIFFE workload identity on a local kind Kubernetes cluster. The demo highlights workload identity with SPIFFE IDs, mutual TLS (mTLS) between services and database, automatic certificate rotation, and policy enforcement via Envoy RBAC - all while keeping application code unchanged.

### High-Level Goals

1. Demonstrate SPIFFE/SPIRE in a production-like manner while remaining runnable on a laptop
2. Show application developers their code stays unchanged while the platform adds strong security guarantees
3. Provide a simple but clear UI showing communication status between components
4. Create a foundation for future extensions (OPA, federation, non-K8s workloads)
...
2. **Given** the system is running, **When** the user inspects Envoy logs, **Then** mTLS handshakes are visible showing SPIFFE IDs being validated
3. **Given** all SPIRE registration entries exist, **When** frontend calls backend, **Then** the backend Envoy validates the frontend's SVID certificate
...
- **FR-004**: Frontend service MUST communicate with backend service through Envoy sidecars using mTLS
- **FR-005**: Backend service MUST communicate with PostgreSQL through Envoy sidecar using mTLS with client certificate authentication
...
- **FR-012**: Application code (frontend and backend) MUST NOT contain SPIRE/SPIFFE-specific libraries in v1 - all identity handling is via Envoy and SPIRE
- **FR-013**: PostgreSQL MUST require client certificate authentication for connections from backend
...
- **Workload**: A running service (frontend, backend) that receives an SVID from SPIRE via the Workload API
...
- **SC-002**: 100% of mTLS connections use valid SVIDs issued by SPIRE (verifiable via Envoy logs)
- **SC-003**: Policy changes (RBAC modification) take effect within 60 seconds of deployment without application restarts
- **SC-004**: Certificate rotation occurs automatically without any service downtime or failed requests
